...
class Router extends React.Component {
  ...
  _updateOneSignalSubscriptionState = async () => {
    await OneSignal.getPermissionSubscriptionState(async status => {
      ...
      if (status.userId != null) {
        ...
        // alert(
        //   "Onesignal failed to register - there was no userID returned in getPermissionSubscriptionState()"
        // );
        toast(
  ...
  _acceptOrderRequest = async (orderRequestId, driverId) => {
    try {
      ...
      if (acceptResult.status == 200) {
        ...
        // alert(
        //   "Driver couldn't accept the order - server returned an error:" +
        //   acceptResult.data.message
        // );
    ...
    if (
      ...
      this.setState({notificationWebsocketRendered: true}, () => {
        ...
        this.rws.addEventListener('open', () => {
          // toast("Websocket open for: " + this.props.user.user.email);
          // showMessage({
          //   message: "Websocket open for: " + this.props.user.user.email,
          //   description:
          //     "Websocket open for: " + this.props.user.user.email,
          //   type: "success",
          //   autoHide: true,
          //   duration: 1000,
          //   position: "bottom",
      ...
      return (
        <View
          ...
          {/* <View
              ...
              <FastImage              
                  source={imgSrc}
                  style={{ minHeight: 36, maxHeight: 36, maxWidth: 36, minWidth: 36 }}
                  resizeMode="contain"
                />
              </View>    */}
        </View>
  ...
  _renderCountOrStoreImg = parentHeightWidth => {
    if (this.state.currentFadeTextIsItems) {
      return (
        // <FadeInOut
        // visible={this.state.currentFade}
        // //visible={true}
        // duration={3000} rotate={true} scale={true}>
  ...
  _renderActiveDriverOrderRequestMenu = () => {
    // showMessage({
    //   position: "bottom",
    //   message: "There are  " + this.state.activeDriverOrderRequests.length + "  driver req" ,
    //   description:
    //     "x/Y: " + this.state.driverx + " " + this.state.drivery,
    //   color: "white", // text color,
    //   autoHide: true,

    // });
    let paddingAmount = 10;

    if (this.state.activeDriverOrderRequests.length > 0) {
      let heightAndWidth = draggableBaseSize;
      let netName = this.state.activeDriverOrderRequests[0].networkName;
      let netColor = this.state.activeDriverOrderRequests[0].networkColor;
      ...
      return (
        <Draggable
          onShortPressRelease={() => this.triggerDriverOrderRequestsModal()}
          renderSize={108}
          ...
          x={this.state.driverx}
          y={this.state.drivery}
          shouldReverse={false}
          pressDragRelease={e => {
            let newX = e.nativeEvent.pageX - e.nativeEvent.locationX;
            let newY = e.nativeEvent.pageY - e.nativeEvent.locationY;

            let bounds = heightAndWidth - 10;
            if (
              (newX >= width + bounds) |
              (newX < -bounds) |
              (newY >= height + bounds) |
              ...
              // alert("that went out of bounds, ignoring!")
            } else {
              this.setState({
                driverx: e.nativeEvent.pageX - e.nativeEvent.locationX,
                drivery: e.nativeEvent.pageY - e.nativeEvent.locationY,
                draggablesHaveBeenMovedSinceStateChange: true,
              });
            }
          ...
          <View
            ...
            style={{
              ...
              backgroundColor: '#02cdc1',
              borderRadius: heightAndWidth / 2,
              maxHeight: heightAndWidth,
              height: heightAndWidth,
              minWidth: heightAndWidth,
              width: heightAndWidth,
              maxWidth: heightAndWidth,
              opacity: 0.82,
              flex: 1,
              zIndex: 9999,
              elevation: 9999,
            ...
            {this._renderMapOrDriverImg(heightAndWidth)}
            {/* <FadeInOut         
          visible={this.state.currentsFade} 
          //visible={true} 
          duration={300} rotate={false} scale={false}>    */}
            <View style={{marginLeft: 8, marginRight: 8}}>
              {/* <Text 
             numberOfLines={2}             
             style={{
  ...
  _renderActiveStoreInboundOrderMenu = () => {
    if (this.state.storeOrderCount > 0) {
      ...
      return (
        <Draggable
          onShortPressRelease={() => this.goToScreen('StoreHomeScreen')}
          renderSize={108}
          renderColor={'transparent'}
          renderShape={'circle'}
          style={{
          ...
          pressDragRelease={e => {
            ...
            if (
              ...
              this.setState({
                storex: e.nativeEvent.pageX - e.nativeEvent.locationX,
                storey: e.nativeEvent.pageY - e.nativeEvent.locationY,
                draggablesHaveBeenMovedSinceStateChange: true,
              });
            }
          ...
          <View
            ...
            <TouchableOpacity
              style={{flex: 1}}
              onPress={() => this.goToScreen('StoreHomeScreen')}
              onLongPress={() => this.goToScreen('StoreHomeScreen')}>
              {this._renderCountOrStoreImg(heightAndWidth)}
              {/* <FadeInOut         
          visible={this.state.currentFade} 
          //visible={true} 
          duration={300} rotate={false} scale={false}>    */}
  ...
  _renderActiveCustomerOrderRequestMenu = () => {
    if (this.state.activeCustomerOrderRequests.length > 0) {
      let heightAndWidth = draggableBaseSize;
      let netName = this.state.activeCustomerOrderRequests[0].networkName;
      let netColor = this.state.activeCustomerOrderRequests[0].networkColor;
      ...
      return (
        <Draggable
          onLongPress={() => this.triggerCustomerOrderRequestsModal()}
          onPressIn={() => this.triggerCustomerOrderRequestsModal()}
          //onPressOut={()=>this.triggerCustomerOrderRequestsModal()}
          onShortPressRelease={() => this.triggerCustomerOrderRequestsModal()}
          minX={0}
          minY={0}
          maxX={width - 10}
          maxY={height - 10}
          ...
          <View
            ...
            {this._renderMapOrLogo(heightAndWidth)}
            {/* <FadeInOut         
          visible={this.state.currentFade} 
          //visible={true} 
          duration={300} rotate={true} scale={true}>            */}
            <TouchableOpacity
              onPress={() => this.triggerCustomerOrderRequestsModal()}
              onLongPress={() => this.triggerCustomerOrderRequestsModal()}
              style={{marginLeft: 8, marginRight: 8, marginBottom: 6}}>
              <Text
                numberOfLines={2}
                style={{
                  marginLeft: 3,
  ...
  componentWillUnmount = () => {
    ...
    //alert("The router.js unmounted!! That's not right");
    ...
    if (typeof this.props.user !== 'undefined') {
      if (
        ...
        if (this.props.user.user.roles.find(x => x === 'Logistics')) {
          // showMessage({
          //   position: "center",
          //   message: "Welcome back superuser!",
          //   autoHide: false,
          //   duration: 2400,
          //   description:
          //     "Have a nice day.",
  ...
  _startResetWebsocketEveryXMinutesTimer = () => {
    let timerId = setInterval(() => {
      try {
        ...
        console.debug('refresh messages crashed');
        // alert("refresh websocket crashed")
      }
    }, 1300000);
    ...
    if (
      ...
      try {
        ...
        if (activeCustomerOrderRequestResponse.status == 404) {
          //clear state if tehre's items in it
          if (this.state.activeCustomerOrderRequests.length > 0) {
  ...
  _refreshActionMessagesAndBlockIfNeeded = async () => {
    //if there's no logged in user, forget it
    if (
      typeof this.props.user.user !== 'undefined' &&
      this.props.user.user != null
    ) {
      ...
      try {
        ...
        if (allMessagesResponse.status == 200) {
          ...
          //take action if there are ANY bloicking messages and show the modal!
          let anyBlockesr = allMessagesResponse.data.filter(
            x => x.blockingRequest == true,
          );
          //take action if there are any driver order requests
          let anyOrderRequests = allMessagesResponse.data.filter(
            x => x.actionToTake == 'DriverOrderRequest',
          );

          if (anyBlockesr.length > 0) {
            //check if modal NOT open already, and if not, play sound and trigger
            if (
              this.props.modalsArray.find(
                x => x.modalName === 'actionMessageModal',
              ).isOpenValue == false
              ...
              //let durationToStop = 90000;
              //set top level timer first to give them a chance
              // setTimeout(() => {
              //   this.triggerActionMessagesModal();
              // }, durationToStop);
            }
          }
          ...
          if (anyOrderRequests.length > 0) {
            ...
            // var timeOfRequest = moment.utc(item.whenCreatedUTC);
            if (true) {
              //make a little noise
              //SoundPlayer.playSoundFile("smbfireball", "mp3");
            }
            ...
            if (this.state.actionMessageRefreshTimerId != -1) {
              ...
              // clearIntervalAsync(this.state.actionMessageRefreshTimerId);
              // this.setState({ actionMessageRefreshTimerId: -1 });
            }
          }
        }
      } catch (error) {
    ...
    if (await MapWorker.requestLocationPermission()) {
      ...
      setTimeout(() => {
        try {
          ...
          //   async (position) => {       // showMessage({
          //     //   position: "center",
          //     //   message: "We got ya!! Let's get to it!",
          //     //   autoHide: false,
          //     //   duration: 1000,
          //     //   description:
          //     //    "",
