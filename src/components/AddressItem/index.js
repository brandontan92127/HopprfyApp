    ...
    return (
      <TouchableOpacity style={[styles.container, {backgroundColor: background}]} activeOpacity={0.85} onPress={onPress}>
        <View style={styles.content}>
          <Text style={[styles.name, {color: text}]}>{item.firstName+" "+item.lastName}</Text>
          <Text style={[styles.text, {color: text}]}>{item.email}</Text>
          <Text style={[styles.text, {color: text}]}>{item.telephone}</Text>
          <Text style={[styles.text, {color: text}]}>{item.zip}</Text>
