...
class CategoryCarousel extends Component {
  ...
  renderItem = ({ item, index }) => {
    ...
    console.debug("redering categoru carousel with item:" + item);
    const image =
      item.image !== null ? { uri: item.image } : Images.categoryPlaceholder;
...
function mergeProps(stateProps, dispatchProps, ownProps) {
  ...
  return {
    ...ownProps,
    ...stateProps,
    fetchCategories: (lastSelectedNetId) => {
      if (!netInfo.isConnected) return toast(Languages.noConnection);
      actions.fetchCategories(dispatch, lastSelectedNetId);
    },
