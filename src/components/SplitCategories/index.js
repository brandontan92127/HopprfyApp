...
class SplitCategories extends Component {
  ...
  renderItem = ({ item }) => {
    ...
    const imageURL =
      item.images.length > 0
        ? getProductImage(item.images[0], Styles.width)
  ...
  selectCategory = index => {
    ...
    clearProducts();
    fetchProductsByCategoryId(categories[index]._id, 1);
  };
  ...
  componentDidMount() {
    ...
    if (categories.length > 0) {
      fetchProductsByCategoryId(categories[0]._id, 1);
    }
  }

  componentWillReceiveProps(nextProps) {
    if (this.props.categories.length == 0 && nextProps.categories.length > 0) {
      this.props.fetchProductsByCategoryId(nextProps.categories[0]._id, 1);
    }
  }
}
