...
export const actions = {
  fetchProductsByCategoryId: async (dispatch, classId, per_page, page) => {   
    return new Promise(async (resolve, reject) => {

      try {        
    dispatch({ type: types.FETCH_PRODUCTS_PENDING });

    //todo - needs to be dynamic not hardcoded id
    const prodResponse = await HopprWorker.productsByClassIdInStockAndLive(
      classId,      
      lat=null,
      lng = null
    );
    ...
    if (prodResponse.status == 200) {
      let json = prodResponse.data;     
      dispatch(actions.fetchProductsSuccess(json));
      resolve();      
    }  else {      
      alert("Couldn't get data from server to update your products")
  ...
  fetchProductsByName: async (
    ...
    const json = await HopprWorker.productsByName(name, per_page, page);
  ...
  fetchAllProducts: async (dispatch, per_page = 20, page = 1) => {
    dispatch({ type: types.FETCH_PRODUCTS_PENDING });
    const json = await HopprWorker.getAllProducts(
      per_page,
      page,
  ...
  fetchProductRelated: async (dispatch, product) => {
    console.debug("In redux, fetching related proudcts");
    dispatch({ type: types.FETCH_PRODUCTS_RELATED_PENDING });

    let classId = product.classId;
    let relatedResponse = await HopprWorker.productsByClassIdInStockAndLive(classId, lat = null, lng = null);
    if(relatedResponse.status == 200)
    {
      let relatedProdsJSON = relatedResponse.data;
      //we just send 0 related prods for now
      dispatch({
        type: types.FETCH_PRODUCTS_RELATED_SUCCESS,
        productRelated: relatedProdsJSON,
      });  
    }
...
export const reducer = (state = initialState, action) => {
  ...
  switch (type) {
    ...
    case types.FETCH_PRODUCTS_RELATED_SUCCESS: {
      console.debug("Fetch product related success!!");
      return Object.assign({}, state, {
        isFetching: false,
